...
// Kind 把流内错误分类：1005（权益不足）→ ErrHardCredit；其余归 ErrClient。
...
// StreamWithError 同 Stream，额外在遇到上游 event:error 时回调 onErr，
// 供调用方冷却账号 / 记录日志。
		...
		if ev := scanLine(st, strings.TrimRight(line, "\r\n")); ev != nil {
			...
			case "output":
				...
				if len(ev.ToolCalls) > 0 && string(ev.ToolCalls) != "null" {
					if tc := convertToolCalls(ev.ToolCalls); tc != nil {
						delta["tool_calls"] = tc
					}
				}
				if len(delta) > 0 {
			...
			case "error":
				...
				// finish_reason 设为 stop 避免客户端持续等待。
				se := &SOLOStreamError{Code: ev.ErrorCode, Msg: ev.ErrorMessage}
				if onErr != nil {
					onErr(se)
				}
				if err := writeChunk(map[string]any{"content": se.Error()}, "stop"); err != nil {
					return err
				}
				if err := writeDONE(); err != nil {
					return err
...
// convertToolCalls 把 SOLO 的 tool_calls 条目转成 OpenAI 标准结构。
func convertToolCalls(raw json.RawMessage) []map[string]any {
