...
「重渲染后积分明细仍保留（当前实测 12 → 12 条）」和
「空闲 25 秒内没有任何 `/api/state` 轮询」。
...
（workbuddy/cn 真实账号，**上游原始返回 24 条**）后**只用其中一个**：
...
**过滤口径：只保留「还有剩余」的权益包。** 判据是 `total > 0 && remain > 0`
（`internal/upstream/client.go` 的 `keepResourceItem`）：

- `total <= 0` —— 这个包本身没有额度
- `remain <= 0` —— 已经用完

两者对合计都是零贡献，留在面板上只是噪音。实测 workbuddy/cn 的 24 条里有
**12 条 `remain = 0`**，改动前面板上就是连续 12 行「0 / 100」。过滤后
24 → **12 条**，合计 **1929 不变** —— 这正是「被滤掉的确实对合计零贡献」的证明。

判据**只看剩余、不看到期时间**：到期时间已经单独显示（过期染红），
不在这里重复判断，避免同一件事有两个口径。

测试锁的是更强的命题「**不会误滤**」：被滤掉的条目 `remain` 必须为 0
（`TestKeepResourceItemOnRealShape`）。若判据退化成「只判 `total > 0`」，
保留数会从 12 变成 24、滤掉数从 12 变成 0，测试立刻红 —— 已做变异验证。

`render-check.js` 侧的过滤断言**遍历该渠道的全部账号**，不只看第一个：
实测 traework/cn 的第一个账号本来就没有 0 行条目，只测它的话这条断言
「是绿的但从没被验证过」（端到端变异验证时它不红）。改成遍历后，
变异版下它立刻报出第二个账号的 `33 条`里有 25 条 `remain = 0`。

> 顺带修掉一处**偶发假红**：WorkBuddy 详情原本用固定 `waitForTimeout(1200)`
> 等明细渲染，而上游响应慢时不够，实测出现过「用量条 0 条」这种看着像回归、
> 实为时序的失败。已改成等 `.det .bar` 出现（20 秒上限），连跑 3 次稳定。

面板侧的空态文案相应改为「没有可用的权益包（上游未返回，或返回的都已用完）」：
过滤之后空列表有两种成因，只写「上游未返回」会在第二种情况下给出错误结论。
...
1. **没有剩余的权益包过滤掉**（与 WorkBuddy 同一口径）。判据是
   `credits_limit > 0 && remain > 0`（`internal/traework/client.go` 的
   `keepResourceItem`）：
   - `credits_limit = 0` —— 实测有一条 `display_desc = "免费"` 的包
   - `remain = 0` —— 已经用完。实测 traework/cn 的**第二个账号 33 条里有 25 条**
     `remain = 0`，面板上就是连续 25 行「0 / 200」

   过滤后 33 → **8 条**，合计 **1200 不变**；另一个账号 12 条本来就有剩余，不受影响。
   锁在 `TestKeepResourceItemOnRealShape`（同样做过变异验证）。
...
| `upstream` | 13.7% | 请求体改写不变量、「今天已签到」识别不吞真实错误、**到期时间的字段取舍与时区**、明细过滤不误滤 |
| `traework` | 4.2% | 模型过滤三条规则、**明细的到期字段与显示名取舍、过滤不误滤**（覆盖率低是因为该包大量代码是 HTTP 调用） |
